...
public:
    Food() {
        ...
        message = U"";
        minStage = 0;
        texturefileName = ""; // 텍스쳐 파일 이름
    }

    Food(u32string name, int canEat, u32string message, int minStage, string texturefileName)
        : name(name), canEat(canEat), message(message), minStage(minStage), texturefileName(texturefileName)
    {

    }

    Food(const Food& f) : name(f.name), canEat(f.canEat), message(f.message), minStage(f.minStage), texturefileName(f.texturefileName)
    { // 음식 교체 
    }
    ...
    void change(const Food& f) { // 음식 교체 
        ...
        message = f.message;
        minStage = f.minStage;
        texturefileName = f.texturefileName;
    }
...
private:
    ...
    vector<Food> foods = {
        ...
        "츄르.jpg"),
        Food(U"통조림", 1, U"고양이 통조림은 고양이가 아주 좋아합니다. \n사람이 먹는 통조림은 고양이가 먹으면 안됩니다!", 0
        , "통조림.jpg"),
